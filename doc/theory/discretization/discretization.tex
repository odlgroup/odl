...
\begin{definition}
 ...
 %
 \begin{enumerate}
  \item \emph{$\Op{T}$ and $\DiscOp{T}$ (exactly) correspond to each 
  ...
  \item \emph{$\Op{T}$ and $\DiscOp{T}$ approximately correspond to each other under the discretizations $\Discr_{n}(\VecSpace{X})$  
...
\begin{lemma}
 Let $\DiscOp{T}$ be the natural discretization of an operator $\Op{T}$ as in \Cref{def:operator_discretization} and
 %
 \begin{equation*}
  \PnX := \EnX \circ \RnX \quad \text{and} \quad \PmY := \EmY \circ \RmY.
 \end{equation*}
 %
 Let further the \emph{domain of injectivity} of an operator $\Op{T}$ defined on $\VecSpace{X}$ be the set
 %
 \begin{equation*}
  \VecSpace{I}(\Op{T}) := \bigcup \{A \subset \VecSpace{X}\, |\, \Op{T} \text{ injective on } A \}.
 \end{equation*}
 %
...
\[  P^{\VecSpace{Y}}_{m} := \EmY \circ \RmY
   \quad\text{and}\quad
   P^{\VecSpace{X}}_{n} := \EnX \circ \RnX.
\]   
Both these operators do in some sense corresponds to the identity mapping but they have a finite-dimensional range.

With these we may re-phrase the conditions for exact correspondence in Definition~\ref{def:operator_compatibility}
in a way that is sometimes more useful. If $\VecSpace{X}$ is a Hilbert space, then
\begin{align*}
  \EmY \circ \DiscOp{T} = \Op{T} \circ \EnX
    &\quad\iff\quad
    \Op{T} = P^{\VecSpace{Y}}_{m} \circ \Op{T}
    \text{ on $\range(\EnX) \subset \VecSpace{X}$} \\
 \RmY \circ \Op{T} = \DiscOp{T} \circ \RnX
    &\quad\iff\quad
    \Op{T} = \Op{T} \circ P^{\VecSpace{X}}_{n}
    \text{ on $\range(\EnX) \subset \VecSpace{X}$}     
\end{align*}

\[  \Op{T} \circ \EnX = \EmY \circ \DiscOp{T} 
      = \EmY \circ \RmY \circ \Op{T} \circ \EnX
\]
which is equivalent to assuming that 
\[  \Op{T} = \EmY \circ \RmY \circ \Op{T}
     \quad\text{on the vector space $\range(\EnX) \subset \VecSpace{X}$.} 
\]
In a similar way, 
