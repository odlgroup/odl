...
\begin{definition}
  ...
  $\discr_{n}$ of $\VecSpace{X}$} is determined by the tuple 
  \[
     \discr_{n} := \bigl\{ \VecSpace{X},\Field^{n},R_{n},E_{n} \bigr\} 
  \]
  where 
  ...
  If the maps $R_{n}$ and $E_{n}$ are linear, then the discretization $\discr_{n}$ is called a 
  \emph{linear discretization}.
\end{definition}
...
\]
Then we can define a natural $N$-dimensional discretization of the vector space 
...
\end{align*}
for $x_i\in \VecSpace{X}_i$ and $a_i \in \Field^{n_i}$ with $i=1,\ldots,k$.   The discretization 
...
\begin{definition}\label{def:MapComp}
  ...
  \end{enumerate}  
\end{definition}
Let us now see the consequences of the above definition for some rather common operations
on vector spaces.
%
%
%Since both $\VecSpace{X}$ and 
%$\Field^{n}$ are vector spaces over $\Field$, it makes sense to understand when the 
%
% Many natural operations 
%on $\VecSpace{X}$, such as vector space group law and multiplication and inner product, can be 
%independently defined in $\VecSpace{X}$ and $\Field^{n}$. Often it 
...
\begin{align*}
  ...
  &= \sum_{i,j=1}^n \alpha_i \beta_j \ip{R_{n}(v_i)}{R_{n}(v_j)}_{n} =
        \sum_{i,j=1}^n \alpha_i \beta_j \ip{\be_i}{\be_j}_{n}
\end{align*}
where $\be_i\in\Field^{n}$ is the vector with $1$ at $i$:th position and zeroes otherwise. 
Hence, we see that $\ip{x}{y}_{\VecSpace{X}} =  \ip{R_{n}(x)}{R_{n}(y)}_{n}$ if and only if 
\[ \ip{v_i}{v_j}_{\VecSpace{X}}=\ip{\be_i}{\be_j}_{n}. \]
Conversely,
...
\[  \discr_{n}(\VecSpace{X}) := \{ \VecSpace{X}, \Field^{n}, R_{n}^{\VecSpace{X}},E_{n}^{\VecSpace{X}} \} 
    \quad\text{and}\quad
    \discr_{m}(\VecSpace{Y}) := \{ \VecSpace{Y}, \Field^{n}, R_{m}^{\VecSpace{X}},E_{m}^{\VecSpace{X}} \}. 
\]    
We now define $\VecSpace{L}(\VecSpace{X},\VecSpace{Y})$ as the vector space of linear bounded maps
between $\VecSpace{X}$ and $\VecSpace{Y}$. Similarly, the vector space of $n \times m$ matrices
$\Field^{n \times m}$ corresponds to a vector space of linear maps from $\Field^{n}$ to $\Field^{m}$.
The question is if there is a natural discretization of $\VecSpace{L}(\VecSpace{X},\VecSpace{Y})$ 
given the discretizations $\discr_{n}(\VecSpace{X})$ and $\discr_{m}(\VecSpace{Y})$ so that 
the vector space operations on $\Field^{n \times m}$ correspond to those of 
$\VecSpace{L}(\VecSpace{X},\VecSpace{Y})$.

A special case is when $\VecSpace{Y}=\Field$, in which case 
$\VecSpace{L}(\VecSpace{X},\VecSpace{Y})=\VecSpace{X}^*$.
