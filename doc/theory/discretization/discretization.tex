...
%
The natural discretization of a product space as given in \Cref{def:product_space_discretization} induces a natural discretization of the 
$k$'th derivative similar to the natural operator discretization in \Cref{def:operator_discretization}.
...
discretization of the operator derivative as in \Cref{def:derivative_discretization} is in fact the Fr\'{e}chet derivative of the natural 
...
%
\begin{lemma}
 Let $\VecSpace{X}$ and $\VecSpace{Y}$ are Hilbert spaces with linear discretizations as in \Cref{def:operator_discretization}, and let
...
We proceed by investigating conditions that a discretization should fulfill. Often, we need to perform various operations on 
$\VecSpace{X}$, and there are corresponding operations on $\Field^{n}$. It is then important that such operations are compatible 
...
\begin{proof}
 ...
 conditions for exact correspondence according to \Cref{def:operator_compatibility} can be rephrased as
 %
...
\begin{theorem}
 ...
 sense of \Cref{def:operator_compatibility} if and only if the discretization is linear.
\end{theorem}
