...
$\sqrt{r^2 - \sigma_{\text{max}}^2} = r\cos(\beta_1\psi_0/2)$. Thus, the conditions on $\psi_0$ and $\vartheta_0$ read as
%
...
\begin{equation}
 ...
 X(\phi, \psi, z) = -(R - r)\, \theta(\phi) + R\, \theta(\phi + \psi) + \tan(z/2r)\, e_z.
\end{equation}
%
...
\begin{equation}
 ...
 N(\phi, \psi, z) = -\theta\big( \phi + \beta\psi/2, \tan(z/2r) \big),
\end{equation} 
%
...
\begin{align}
 &\OPD: L^2(\Omega) \Lto L^2(T \times U) \notag \\
 &\OPD f(\phi, \psi, z) = \int_0^\infty f\left( -r\theta(\phi) + s \theta\big( \phi + \beta\psi/2, \tan(z/2r) \big) \right)\, \D s.
\end{align}
%
We use its relation to the two-folded curved operator $\OPD_{\mathrm{TC}}$ as defined in \eqref{eq:conecurved:fwdproj} and proceed as in 
section \ref{sec:applications:fanbeam:flat} to get the backprojection:
%
