...
G = odl.BroadcastOperator(*[odl.applications.tomo.RayTransform(X, gi, impl='astra_cpu')
                            for gi in geometry])
...
if not os.path.exists(file_data):
    ...
    data = odl.core.phantom.poisson_noise(factors * sinogram + background,
                                     seed=1807)
...
g = spdhg.TotalVariationNonNegative(X, alpha=5e-2, strong_convexity=5e-1)
...
if not os.path.exists(file_target):
    ...
    spdhg.save_signal(suby_opt[0], 'suby_saddle[0]', folder_main, 4)

else:
    (x_opt, y_opt, subx_opt, suby_opt, obj_opt) = np.load(file_target)

# set distances
dist_x = 1 / 2 * odl.functionals.L2NormSquared(X).translated(x_opt)
dist_y = 1 / 2 * odl.functionals.L2NormSquared(Y).translated(y_opt)
