...
External packages can add implementations of `BaseTensorSet` and
`BaseTensorSpace` by hooking into the setuptools entry point ``'odl.space'``
...
TENSOR_SET_IMPLS: dict
    A dictionary that maps a string to an `BaseTensorSet` implementation.
TENSOR_SPACE_IMPLS: dict
    A dictionary that maps a string to a `BaseTensorSpace` implementation.

Notes
-----
This is used with functions such as `rn`, `fn`, `rtensors` and
`uniform_discr` in order to allow arbitrary implementations.

See Also
--------
NumpyTensorSpace : Numpy based implementation of `BaseTensorSpace`
NumpyTensorSet : Numpy based implementation of `BaseTensorSet`
"""
