...
def index_of_cuda_device(device: "torch.device"):
    ...
    if device == "cpu":
        return None
    return int(str(device).rsplit(":", maxsplit=1)[-1])
...
class AstraCudaImpl:
    ...
    def __init__(self, geometry, vol_space, proj_space):
        ...
        if isinstance(geometry, Parallel3dAxisGeometry) and not astra_supports(
            ...
            for i, angle in enumerate(geometry.angles):
                if abs(np.dot(axis, geometry.det_to_src(angle, mid_pt))) < 1e-4:
                    warnings.warn(
                        f"angle {i}: detector midpoint normal {geometry.det_to_src(angle, mid_pt)} is perpendicular to the geometry axis {axis} in `Parallel3dAxisGeometry`; this is broken in ASTRA {ASTRA_VERSION}, please upgrade to ASTRA {req_ver}",
                        RuntimeWarning,
                    )
                    break
    ...
    def _call_forward_real(self, vol_data: DiscretizedSpaceElement, out=None, **kwargs):
        ...
        with self._mutex:
            ...
            if out is not None:
                ...
                ), f"The out argument provided is a {type(out)}, which is not an element of the projection space {self.proj_space.real_space}"
                if self.vol_space.impl == "pytorch":
                    warnings.warn(
                        "You requested an out-of-place transform with PyTorch. This will require cloning the data and will allocate extra memory",
                        RuntimeWarning,
                    )
            ...
            if out is not None:
                out.data[:] = (
                    proj_data if self.proj_space.impl == "numpy" else proj_data.clone()
                )
            else:
    ...
    def _call_backward_real(
        self, proj_data: DiscretizedSpaceElement, out=None, **kwargs
    ):
        """Run an ASTRA back-projection on the given data using the GPU.
        ...
        with self._mutex:
            ...
            if out is not None:
                out[:] = (
                    volume_data
                    if self.vol_space.impl == "numpy"
                    else volume_data.clone()
                )
                return out
...
def astra_cuda_fp_scaling_factor(geometry):
    ...
    """
    if isinstance(geometry, Parallel2dGeometry) and parse_version(
        ASTRA_VERSION
    ) < parse_version("1.9.9.dev"):
...
def astra_cuda_bp_scaling_factor(proj_space, vol_space, geometry):
    ...
    if parse_version(ASTRA_VERSION) < parse_version("1.8rc1"):
        ...
        if isinstance(geometry, Parallel2dGeometry):
            ...
            isinstance(geometry, FanBeamGeometry)
            ...
            isinstance(geometry, ConeBeamGeometry)
            and geometry.det_curvature_radius is None
        ):
            # Scales with 1 / cell_volume
            # In 1.7, only cubic voxels are supported
        ...
        if isinstance(geometry, Parallel2dGeometry):
            ...
            scaling_factor *= src_radius**2 * det_px_area**2 / vol_space.cell_volume**2
