...
def test_factory(exponent, tensor_space_impl):
    discr = odl.uniform_discr(0, 1, 10, impl=tensor_space_impl,
                              exponent=exponent)

    assert isinstance(discr.dspace, BaseTensorSpace)
    assert discr.dspace.impl == tensor_space_impl
    assert discr.is_rn
    ...
    try:
        discr = odl.uniform_discr(0, 1, 10, dtype='complex',
                                  impl=tensor_space_impl, exponent=exponent)
...
def test_factory_dtypes(tensor_space_impl):
...
def test_element_from_array_1d():
    ...
    assert isinstance(elem, odl.DiscreteLpElement)
    assert isinstance(elem.ntuple, NumpyTensor)
...
def test_zero():
    ...
    assert isinstance(zero.ntuple, NumpyTensor)
...
def test_operators(tensor_space_impl):
...
def test_ufunc(tensor_space_impl, ufunc):
    space = odl.uniform_discr([0, 0], [1, 1], (2, 2), impl=tensor_space_impl)
...
def test_reduction(tensor_space_impl, reduction):
    space = odl.uniform_discr([0, 0], [1, 1], [2, 2], impl=tensor_space_impl)
...
def test_power(tensor_space_impl, power):
    space = odl.uniform_discr([0, 0], [1, 1], [2, 2], impl=tensor_space_impl)
    ...
    if int(power) != power and tensor_space_impl == 'cuda':
        with pytest.raises(ValueError):
            x_pos ** power
        with pytest.raises(ValueError):
            x_pos **= power
...
def test_norm_rectangle_boundary(tensor_space_impl, exponent):
    ...
    discr = odl.uniform_discr_fromspace(
        ...
        impl=tensor_space_impl, nodes_on_bdry=True)
    ...
    discr = odl.uniform_discr_fromspace(
        fspace, (4, 8), exponent=exponent,
        impl=tensor_space_impl, nodes_on_bdry=((False, True), False))
    ...
    dspace = odl.rn(part.size, dtype=dtype, impl=tensor_space_impl,
                    exponent=exponent,
                    weight=weight)
