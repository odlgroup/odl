...
def test_forward_backward_input_handling():
    ...
    h = odl.functional.ZeroFunctional(space1)
...
def test_forward_backward_basic():
    ...
    g = [odl.functional.ZeroFunctional(space)]
    f = odl.functional.ZeroFunctional(space)
    h = odl.functional.L2NormSquared(space)
...
def test_forward_backward_with_lin_ops():
    ...
    g = [odl.functional.L2NormSquared(space)]
    f = odl.functional.ZeroFunctional(space)

    # Gradient of two-norm square
    h = odl.functional.L2NormSquared(space).translated(b)
...
def test_forward_backward_with_li():
    ...
    f = odl.functional.ZeroFunctional(space)
    h = odl.functional.ZeroFunctional(space)
    l = [0.5 * odl.functional.L2NormSquared(space)]
...
def test_forward_backward_with_li_and_h():
    ...
    g = [odl.functional.IndicatorBox(space, lower=lower_lim, upper=upper_lim)]
    f = odl.functional.ZeroFunctional(space)
    h = 0.5 * odl.functional.L2NormSquared(space)
    l = [0.5 * odl.functional.L2NormSquared(space)]
