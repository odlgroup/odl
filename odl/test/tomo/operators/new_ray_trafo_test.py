...
def test_projector(projector, in_place):
    ...
    assert proj.data.max() == pytest.approx(expected_max, rel=rtol)
...
def test_angles(projector):
    ...
    if isinstance(result, np.ndarray):
        ind_angle:int = int(np.argmax(np.max(result, axis=axes)))
    elif isinstance(result, torch.Tensor):
        ind_angle:int = int(torch.argmax(torch.amax(result, dim=axes)).item())
    ...
    if isinstance(result, np.ndarray):
        ...
        if projector.domain.ndim == 2:
            ind_pixel = torch.argmax(result[ind_angle], dim=0).item() 
        else:
            ind_pixel = int(torch.argmax(torch.amax(result[ind_angle], dim=1)))
...
def test_shifted_volume(geometry_type):
    ...
    space = odl.uniform_discr(min_pt + shift, max_pt + shift, [10] * ndim, impl=impl)
    ray_trafo = odl.tomo.RayTransform(space, geometry)
...
def test_detector_shifts_3d(shifts_3d):
    ...
    if isinstance(y, np.ndarray):
        ...
        # check forward pass 
        data_error = torch.max(torch.abs(y[:, :-k, l:] - y_shift[:, k:, :-l]))
        assert data_error < 1e-3
        # check back-projection
        error = torch.max(torch.abs(im_shift - im))
        assert error < 1e-3
