...
"""Test to make sure the BaseTensorSpace spaces work with larger sizes."""
...
def tspace(tensor_space_impl, request):
    spc = request.param

    if spc == 'rn':
        return odl.rn(10 ** 5, impl=tensor_space_impl)
    elif spc == '1d':
        return odl.uniform_discr(0, 1, 10 ** 5, impl=tensor_space_impl)
    elif spc == '3d':
        return odl.uniform_discr([0, 0, 0], [1, 1, 1],
                                 [100, 100, 100], impl=tensor_space_impl)
