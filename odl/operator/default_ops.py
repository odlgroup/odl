...
class ScalingOperator(Operator):
    ...
    def _call(self, x, out=None):
        ...
        if out is None:
            ...
            self.range.lincomb(self.scalar, x, out=out)
        return out
    ...
    def adjoint(self):
        ...
        >>> expected_op = odl.ScalingOperator(c3, 1 - 1j)
...
class MultiplyOperator(Operator):
    ...
    def __init__(self, domain, multiplicand, range=None):
        ...
        multiplicand : `array-like` or scalar
            Value to multiply by.
        range : `LinearSpace` or `Field`, optional
            Set to which the operator maps. Default: ``domain``.
        ...
        >>> op = odl.MultiplyOperator(r3.field, x, range=r3)
        >>> op(3)
        array([ 3.,  6.,  9.])
        >>> out = r3.element()
        >>> op(3, out)
    ...
    def _call(self, x, out=None):
        ...
        if out is None:
            ...
            if self.__domain_is_field:
                self.range.lincomb(x, self.multiplicand, out=out)
            else:
                self.range.multiply(x, self.multiplicand, out=out)
        else:
    ...
    def adjoint(self):
        ...
        >>> op = odl.MultiplyOperator(r3, 3.0, range=r3)
        >>> op.adjoint(x)
        array([ 3.,  6.,  9.])

        Multiplication operator on complex space:

        >>> c3 = odl.cn(3)
        >>> x_complex = c3.element([1, 1j, 1-1j])
        >>> op = odl.MultiplyOperator(c3, x_complex)
        >>> op.adjoint.multiplicand
        ...
        if self.__domain_is_field:
            ...
            return MultiplyOperator(
                self.range, self.multiplicand.conjugate(), range=self.domain
            )
        else:
            return MultiplyOperator(
                self.range, self.multiplicand, range=self.domain
            )
...
class PowerOperator(Operator):
    ...
    def derivative(self, point):
        ...
        return self.exponent * MultiplyOperator(
            self.domain, point ** (self.exponent - 1), self.range
        )
...
class InnerProductOperator(Operator):
    ...
    def __init__(self, space, vector):
        ...
        >>> r3 = odl.rn(3)
        >>> x = r3.element([1, 2, 3])
        >>> op = odl.InnerProductOperator(r3, x)
        >>> op([1, 2, 3])
        14.0
        """
    ...
    def _call(self, x):
        ...
        return self.domain.inner(x, self.vector)
...
class NormOperator(Operator):
    ...
    def derivative(self, point):
        ...
        """
        point = self.domain.element(point)
        norm = self.domain.norm(point)
        if norm == 0:
            raise ValueError('not differentiable in 0')

        return InnerProductOperator(self.domain, point / norm)
...
class DistOperator(Operator):
    ...
    def _call(self, x):
        """Return the distance from ``x`` to the fixed vector."""
        return self.domain.dist(x, self.vector)
    ...
    def derivative(self, point):
        ...
        dist = self.domain.dist(point, self.vector)
        if dist == 0:
...
class ConstantOperator(Operator):
    ...
    def __init__(self, range, constant, domain=None):
        ...
        domain : `LinearSpace`, optional
            Domain of the operator. Default: ``range``

        Examples
        --------
        >>> r3 = odl.rn(3)
        >>> x = r3.element([1, 2, 3])
        >>> op = odl.ConstantOperator(r3, x)
        >>> op(x)
        array([ 1.,  2.,  3.])
        """
        if not isinstance(range, LinearSpace):
            raise TypeError(
                '`range` must be a `LinearSpace`, got {!r}'.format(range)
            )
        if domain is None:
            domain = range
...
class ZeroOperator(Operator):
    ...
    def __init__(self, range, domain=None):
        """Initialize a new instance.
        ...
        >>> op = odl.ZeroOperator(odl.cn(4), domain=odl.rn(3))
    ...
    def _call(self, x, out=None):
        ...
        if self.domain == self.range:
            if out is None:
                ...
                self.range.lincomb(0, x, out=out)
        else:
            zero = self.range.zero()
            if out is None:
                out = zero
            else:
    ...
    def adjoint(self):
        ...
        """
        return ZeroOperator(self.domain, domain=self.range)

    def __repr__(self):
        """Return ``repr(self)``."""
        return '{}({!r})'.format(self.__class__.__name__, self.range)
...
class RealPart(Operator):
    ...
    def inverse(self):
        ...
        This is not a true inverse, only a pseudoinverse, the imaginary part
    ...
    def adjoint(self):
        ...
        >>> op.domain.inner(x, op.adjoint(y)) == op.range.inner(op(x), y)
        True
        ...
        >>> y = op.range.element([3, 2, 1])
        >>> AAtxy = op.range.inner(op(op.adjoint(x)), y)
        >>> AtxAty = op.domain.inner(op.adjoint(x), op.adjoint(y))
        >>> AAtxy == AtxAty
        True
        """
        if self.space_is_real:
...
class ImagPart(Operator):
    ...
    def adjoint(self):
        ...
        >>> x = op.domain.element([1, 2, 3])
        >>> y = op.range.element([3, 2, 1])
        >>> op.domain.inner(x, op.adjoint(y)) == op.domain.inner(op(x), y)
        True
...
class ComplexEmbedding(Operator):
    ...
    def adjoint(self):
        ...
        >>> Axy = op.range.inner(op(x), y)
        >>> xAty = op.domain.inner(x, op.adjoint(y))
        >>> Axy == xAty
        True
        ...
        >>> x = r3.element([1, 2, 3])
        >>> y = r3.element([3, 2, 3])
        >>> AtAxy = op.domain.inner(op.adjoint(op(x)), y)
        >>> AxAy = op.range.inner(op(x), op(y))
        >>> AtAxy == AxAy
        True
        """
        if self.domain.is_real:
...
class ComplexModulus(Operator):
    ...
    def _call(self, x):
        """Return ``self(x)``."""
        # TODO(kohr-h): generalize to other array types
