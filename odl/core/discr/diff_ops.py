...
_ADJ_METHOD = {"central": "central", "forward": "backward", "backward": "forward"}
    ...
    def derivative(self, point=None):
        ...
        if self.pad_mode == "constant" and self.pad_const != 0:
            return PartialDerivative(
                self.domain, self.axis, self.range, self.method, self.pad_mode, 0
            )
        return self
...
class Gradient(PointwiseTensorFieldOperator):
    ...
    def __repr__(self):
        ...
        posargs = [self.domain]
        optargs = [
            ("range", self.range, self.domain**self.domain.ndim),
            ("method", self.method, "forward"),
            ("pad_mode", self.pad_mode, "constant"),
            ("pad_const", self.pad_const, 0),
        ]
...
class Divergence(PointwiseTensorFieldOperator):
    ...
    def __repr__(self):
        ...
        posargs = [self.domain]
        optargs = [
            ("range", self.range, self.domain[0]),
            ("method", self.method, "forward"),
            ("pad_mode", self.pad_mode, "constant"),
            ("pad_const", self.pad_const, 0),
        ]
...
class Laplacian(PointwiseTensorFieldOperator):
    ...
    def __init__(self, domain, range=None, pad_mode="constant", pad_const=0):
        ...
        if pad_mode not in _SUPPORTED_PAD_MODES:
            raise ValueError(f"`pad_mode` {pad_mode_in} not understood")
        if pad_mode in ("order1", "order1_adjoint", "order2", "order2_adjoint"):
    ...
    def derivative(self, point=None):
        ...
        if self.pad_mode == "constant" and self.pad_const != 0:
            return Laplacian(
                self.domain, self.range, pad_mode=self.pad_mode, pad_const=0
            )
        return self
    ...
    def adjoint(self):
        ...
        """
        return Laplacian(self.range, self.domain, pad_mode=self.pad_mode, pad_const=0)
...
def _finite_diff_numpy(
    f_arr, axis, dx=1.0, method="forward", out=None, pad_mode="constant", pad_const=0
):
...
def _finite_diff_pytorch(
    f_arr, axis, dx=1.0, method="forward", pad_mode="constant", pad_const=0
):
    ...
    f_arr = f_arr.reshape(
        [prod(orig_shape[:axis]), 1, orig_shape[axis], prod(orig_shape[axis + 1 :])]
    )
    ...
    if pad_mode == "constant":
        if pad_const == 0:
            ...
            result = torch.conv2d(
                torch.cat([padding_arr, f_arr, padding_arr], dim=-2),
                fd_kernel,
                padding="valid",
            )
...
def finite_diff(
    f, axis, dx=1.0, method="forward", out=None, pad_mode="constant", pad_const=0
):
