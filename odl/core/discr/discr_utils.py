...
scipy.interpolate.RegularGridInterpolator.html>`_ class.
    ...
    def _evaluate(self, indices, norm_distances, out=None):
        ...
        for i, yi in zip(indices, norm_distances):
            idx_res.append(self.namespace.where(yi < 0.5, i, i + 1))
...
class _PerAxisInterpolator(_Interpolator):
    ...
    def _evaluate(self, indices, norm_distances, out=None):
        ...
        if out is None:
            ...
            out = self.namespace.zeros(out_shape, dtype=out_dtype, device=self.device)
        else:
            out[:] = 0.0
        ...
        for lo_hi, edge in zip(
            product(*([["l", "h"]] * len(indices))), product(*edge_indices)
        ):
            ...
            out += (
                self.backend.array_constructor(self.values[edge], device=self.device)
                * weight[vslice]
            )
...
def _check_func_out_arg(func):
    ...
    if "out" in pos_args:
        has_out = True
        out_optional = pos_args.index("out") >= len(pos_args) - len(pos_defaults)
    elif "out" in kw_only:
...
def _broadcast_nested_list(arr_lists, element_shape, ndim, backend: ArrayBackend):
    ...
    return [
        _broadcast_nested_list(row, element_shape, ndim, backend=backend)
        for row in arr_lists
    ]


def _send_nested_list_to_backend(arr_lists, backend: ArrayBackend, device, dtype):
    if backend.impl == "numpy":
        return arr_lists
    ...
    if isinstance(arr_lists, (tuple, list)):
        return [
            _send_nested_list_to_backend(arr, backend, device, dtype)
...
def sampling_function(
    ...
    def _sanitise_input_function(func: Callable):
        ...
        if isinstance(func, (list, tuple)):
            raise NotImplementedError(
                "The sampling function cannot be instantiated with a list-like of callables."
            )

        raise NotImplementedError(
            "The function to sample must be either a Callable or an array-like (list, tuple) of callables."
        )
