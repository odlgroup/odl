...
TYPE_PROMOTION_COMPLEX_TO_REAL = {"complex64": "float32", "complex128": "float64"}

DTYPE_SHORTHANDS = {bool: "bool", int: "int32", float: "float64", complex: "complex128"}
...
def real_dtype(
    ...
    if default is None:
        raise ValueError(f"no real counterpart exists for `dtype` {dtype}")
...
def complex_dtype(
    ...
    if default is None:
        raise ValueError(f"no complex counterpart exists for `dtype` {dtype}")
