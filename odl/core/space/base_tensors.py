...
class TensorSpace(LinearSpace):
    ...
    def _init_weighting(self, **kwargs):
        """Internal helper to initialise the weighting
        """
        ...
        if weighting is None:
            ...
            if issubclass(type(weighting), Weighting):
                ...
                if (
                    isinstance(weighting, ArrayWeighting)
                    and weighting.device != self.device
                ):
                    raise ValueError(
    ...
    def array_backend(self) -> ArrayBackend:
        """returns the ArrayBackend of a TensorSpace

        Returns:
            ArrayBackend: Array Backend of the TensorSpace
        """
    ...
    def astype(self, dtype):
        ...
        if is_real_floating_dtype(dtype_identifier) or is_complex_dtype(
            dtype_identifier
        ):
    ...
    def element(self, inp=None, copy=None):
        ...
        if isinstance(inp, LinearSpaceElement) and not isinstance(inp, Tensor):
            raise TypeError(
                "Trying to generated a `Tensor` from an ODL object with more structure, {type(inp)=}"
            )
        ...
        def wrapped_array(arr):
            ...
            if is_real_dtype(self.dtype_identifier) and not is_real_dtype(
                self.array_backend.get_dtype_identifier(array=arr)
            ):
                raise TypeError(
                    f"A real space cannot have complex elements. Got {arr.dtype}"
                )
            if is_int_dtype(self.dtype_identifier) and not is_int_dtype(
                self.array_backend.get_dtype_identifier(array=arr)
            ):
                raise TypeError(
                    f"An integer space can only have integer elements. Got {arr.dtype}"
                )
        ...
        # ---> The data of the input is transferred to the space's device and data type AND wrapped into the space.
            ...
            if inp.space == self and not copy:
        ...
        # ---> The input is broadcasted to the space's shape and transferred to the space's device and data type AND wrapped into the space.
            ...
            raise ValueError(
                f"The input {inp} with dtype {type(inp)} is not supported by the `element` method. The only supported types are int, float, complex, list, tuples, objects with an __array__ attribute of a supported backend (e.g np.ndarray and torch.Tensor) and ODL Tensors."
            )
    ...
    def _elementwise_num_operation(
        self,
        operation: str,
        ...
        **kwargs,
        ...
        if not isinstance(
            x1, (int, float, complex, Tensor, ProductSpaceElement, Operator)
        ):
            raise TypeError(
                f"The type of the left operand {type(x1)} is not supported."
            )

        if not isinstance(
            x2, (int, float, complex, Tensor, ProductSpaceElement, Operator)
        ):
            raise TypeError(
                f"The type of the right operand {type(x2)} is not supported."
            )
        ...
        if isinstance(x1, (int, float, complex)) or isinstance(
            x2, (int, float, complex)
        ):
            if out is None:
        ...
        if isinstance(x1, ProductSpaceElement):
            ...
            return x1.space._elementwise_num_operation(
                operation, x1, x2, out, namespace=namespace, **kwargs
            )
        ...
        if isinstance(x2, Operator):
            if operation == "multiply":
                warnings.warn(
                    "The composition of a LinearSpaceElement and an Operator using the * operator is deprecated and will be removed in future ODL versions. Please replace * with @."
                )
            ...
            raise TypeError(
                f"Attempted numerical operation {operation} between two incompatible objects ({type(x1)=}, {type(x2)=})"
            )
        ...
        if isinstance(x1, Tensor) and isinstance(x2, Tensor):
            ...
            # for instance, if two spaces have a int dtype, the result of the division of one of their element by another return should be of float dtype
            return x1.space.astype(
                x1.space.array_backend.get_dtype_identifier(array=result)
            ).element(result)
        raise TypeError(
                f"Neither x1 nor x2 are odl ODL Tensors. Got {type(x1)} and {type(x2)}"
            )

    def _element_reduction(self, operation: str, x: "Tensor", **kwargs):
        function = getattr(self.array_namespace, operation)
        result = function(x.data, **kwargs)
        try:
            return result.item()
...
class Tensor(LinearSpaceElement):
    ...
    @property
    def array_backend(self) -> ArrayBackend:
        """ArrayBackend object of the Tensor

        Returns:
            ArrayBackend: ODL ArrayBackend object
        """
    ...
    def to_impl(self, impl: str):
        ...
        ), f"Error when transferring array from {self.impl} to {impl}: device changed from {self.device} to {new_data.device}. Ensure to use a device supported by both backends."
        assert (
            _universal_dtype_identifier(new_data.dtype) == self.dtype_identifier
        ), f"Error when transferring array from {self.impl} to {impl}: dtype changed from {self.dtype} to {new_data.dtype}. Ensure to use a dtype supported by both backends."
