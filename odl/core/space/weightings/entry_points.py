...
def space_weighting(impl: str, device="cpu", **kwargs):
    ...
    if "inner" in kwargs:
        ...
        for arg in ["norm", "dist", "weight"]:
            if arg in kwargs:
                raise ValueError(
                    f"If a custom inner product is specified, the weighting cannot also have custom {arg}={kwargs[arg]}."
                )
    ...
    if "norm" in kwargs:
        ...
        for arg in ["exponent", "inner", "dist", "weight"]:
            if arg in kwargs:
                raise ValueError(
                    f"If a custom norm is specified, the weighting cannot also have custom {arg}={kwargs[arg]}."
                )
    ...
    if "dist" in kwargs:
        ...
        for arg in ["exponent", "inner", "norm", "weight"]:
            if arg in kwargs:
                raise ValueError(
                    f"If a custom distance is specified, the weighting cannot also have custom {arg}={kwargs[arg]}."
                )
    ...
    if "weight" in kwargs:
        ...
        for arg in ["inner", "norm", "dist"]:
            if arg in kwargs:
                raise ValueError(
                    f"If a custom weight is specified, the weighting cannot also have custom {arg}={kwargs[arg]}."
                )
        ...
        if isinstance(weight, (int, float)):
            ...
            return ConstWeighting(
                const=weight, impl=impl, device=device, exponent=exponent
            )
        ...
        if hasattr(weight, "__array__"):
            ...
            if backend.array_namespace.all(0 < weight):
                ...
                ), f"The weighing is expecting the device {device}, but the array provided for the weight has a device {weight.device}. Please make sure that the two devices are consistent"
            else:
                raise ValueError(
                    "If the weight is an array, all its elements must be positive"
                )
            ...
            raise ValueError(
                "A weight can only be a positive __array__, a positive float."
            )
