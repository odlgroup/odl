...
class OperatorTest:
    ...
    def self_adjoint(self):
        ...
        with fail_counter(
            ...
            for [name_x, x], [name_y, y] in samples(
                self.operator.domain, self.operator.range
            ):
                x_norm = x.norm()
                y_norm = y.norm()
    ...
    def _derivative_convergence(self):
        ...
        with fail_counter(
            ...
            for [name_x, x], [name_dx, dx] in samples(
                self.operator.domain, self.operator.domain
            ):
    ...
    def _scale_invariance(self):
        ...
        with fail_counter(
            ...
            for [name_x, x], [_, scale] in samples(
                self.operator.domain, self.operator.domain.field
            ):
                opx = self.operator(x)
    ...
    def linear(self):
        ...
        if result_norm != 0.0:
            print("||A(0)||={result_norm:6.5f}. Should be 0.0000")
