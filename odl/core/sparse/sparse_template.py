...
"""ODL Dataclass for multi-backend Sparse Matrices support."""
...
class SparseMatrixFormat:
    """Data class for Sparse Matrices in ODL
    The class has a post-init method that registers the Sparse Matrix format in a dict with [impl][format] key structure, for instance ['scipy']['COO']

    Attributes
    ----------
    sparse_format : str
        The layout of the sparse matrix, e.g 'COO'
    impl : str
        The implementation of the backend, e.g 'scipy'
    constructor : Callable
        Constructor of the Sparse backend
    is_of_this_sparse_format : Callable
    ...
    def __post_init__(self):
        ...
        if self.sparse_format in _registered_sparse_formats[self.impl]:
            raise KeyError(
                f"A {self.sparse_format} sparse format for backend {self.impl} is already registered. Every sparse format needs to have a unique identifier combination."
            )
