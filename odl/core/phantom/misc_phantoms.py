...
def text(space, text, font=None, border=0.2, inverted=True):
    ...
    **Linux**::
    ...
    init_size = 1000
    init_pil_font = ImageFont.truetype(font + ".ttf", size=init_size, encoding="unic")
    ...
    size = scaled_init_size * min(
        [space.shape[0] / init_text_width, space.shape[1] / init_text_height]
    )
    size = int(size)
    ...
    offset = ((space.shape[0] - text_width) // 2, (space.shape[1] - text_height) // 2)
    white = "#000000"
