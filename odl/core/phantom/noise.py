...
__all__ = ("white_noise", "poisson_noise", "salt_pepper_noise", "uniform_noise")
...
def salt_pepper_noise(
    vector, fraction=0.05, salt_vs_pepper=0.5, low_val=None, high_val=None, seed=None
):
    ...
    with npy_random_seed(seed):
        if isinstance(vector.space, ProductSpace):
            values = [
                salt_pepper_noise(
                    subintensity, fraction, salt_vs_pepper, low_val, high_val
                )
            ...
            pepper_indices = a[
                int(fraction * vector.size * salt_vs_pepper) : int(
                    fraction * vector.size
                )
            ]
