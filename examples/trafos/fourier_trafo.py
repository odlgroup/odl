...
ft_op.range.show(phantom_ft, title='Full Fourier Transform')

# Calculate the inverse transform.
phantom_ft_inv = ft_op.inverse(phantom_ft)
space.show(phantom_ft_inv, title='Full Fourier Transform Inverted')
...
ft_op_axis0.range.show(
    phantom_ft_axis0, title='Fourier Transform Along Axis 0'
)
...
space.show(phantom_real, title='Shepp-Logan Phantom, Real Version')
phantom_real_ft = ft_op_halfc(phantom_real)
ft_op_halfc.range.show(
    phantom_real_ft, title='Half-complex Fourier Transform'
)

# If the space is real, the inverse also gives a real result.
phantom_real_ft_inv = ft_op_halfc.inverse(phantom_real_ft)
space.show(
    phantom_real_ft_inv, title='Half-complex Fourier Transform Inverted'
)
...
padded_ft_op.range.show(
    phantom_ft_padded, 'Padded FT of The Phantom', force_show=True
)
