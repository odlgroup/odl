...
ray_trafo_impl = simple_fixture(
    name='ray_trafo_impl',
    params=[
        pytest.param('astra_cuda_link', marks=skip_if_no_astra),
...
reco_space_impl = simple_fixture(
    name='reco_space_impl',
    params=[
        pytest.param('numpy'),
...
def geometry(request):
    ...
    if geom == 'par2d':
        ...
        return odl.tomo.ConeBeamGeometry(apart, dpart, pitch=5,
                                         src_radius=200, det_radius=100)
    else:
...
projectors.extend(
    (pytest.param(proj_cfg, marks=skip_if_no_astra)
     for proj_cfg in [
        # 'par2d astra_cpu numpy uniform',
        # 'par2d astra_cpu numpy nonuniform',
        # 'par2d astra_cpu numpy random',
        # 'cone2d astra_cpu numpy uniform',
        # 'cone2d astra_cpu numpy nonuniform',
        # 'cone2d astra_cpu numpy random'
        ])
)
projectors.extend(
    (pytest.param(proj_cfg, marks=skip_if_no_astra_cuda)
     for proj_cfg in [
        'par2d astra_cuda numpy uniform',
        'par2d astra_cuda numpy half_uniform',
        'par2d astra_cuda numpy nonuniform',  # eroor
        'par2d astra_cuda numpy random',
        'cone2d astra_cuda numpy uniform',
        'cone2d astra_cuda numpy nonuniform',
        'cone2d astra_cuda numpy random',
        'par3d astra_cuda numpy uniform',
        'par3d astra_cuda numpy nonuniform',  # fails
        'par3d astra_cuda numpy random',
        'cone3d astra_cuda numpy uniform',    # fails
        'cone3d astra_cuda numpy nonuniform', # fails
        'cone3d astra_cuda numpy random',
        'helical astra_cuda numpy uniform',

        'par2d astra_cuda pytorch uniform',
        'par2d astra_cuda pytorch half_uniform',
        'par2d astra_cuda pytorch nonuniform',
        'par2d astra_cuda pytorch random',
        'cone2d astra_cuda pytorch uniform',
        'cone2d astra_cuda pytorch nonuniform',
        'cone2d astra_cuda pytorch random',
        'par3d astra_cuda pytorch uniform',
        'par3d astra_cuda pytorch nonuniform',
        'par3d astra_cuda pytorch random',
        'cone3d astra_cuda pytorch uniform',
        'cone3d astra_cuda pytorch nonuniform',
        'cone3d astra_cuda pytorch random',
        'helical astra_cuda pytorch uniform'

        # Only 3D tests so far
        'par3d astra_cuda_pytorch pytorch uniform',
        'par3d astra_cuda_pytorch pytorch nonuniform',
        'par3d astra_cuda_pytorch pytorch random',
        'cone3d astra_cuda_pytorch pytorch uniform',
        'cone3d astra_cuda_pytorch pytorch nonuniform',
        'cone3d astra_cuda_pytorch pytorch random',
        'helical astra_cuda_pytorch pytorch uniform',

        'par3d astra_cuda_pytorch numpy uniform',
        'par3d astra_cuda_pytorch numpy nonuniform',
        'par3d astra_cuda_pytorch numpy random',
        'cone3d astra_cuda_pytorch numpy uniform',
        'cone3d astra_cuda_pytorch numpy nonuniform',
        'cone3d astra_cuda_pytorch numpy random',
        'helical astra_cuda_pytorch numpy uniform'
        ])
)
projectors.extend(
    (pytest.param(proj_cfg, marks=skip_if_no_skimage)
     for proj_cfg in ['par2d skimage numpy uniform',
                      'par2d skimage numpy half_uniform'])
)

projector_ids = [
    " geom='{}' - ray_trafo_impl='{}' - reco_space_impl='{}' - angles='{}' ".format(*p.values[0].split())
    for p in projectors
]
...
def projector(request):
    ...
    geom, ray_trafo_impl, reco_space_impl, angle = request.param.split()
    ...
    if geom == 'par2d':
        ...
        # Ray transform
        return odl.tomo.RayTransform(reco_space, geom, impl=ray_trafo_impl)
        ...
        reco_space = odl.uniform_discr([-20, -20, 0], [20, 20, 40],
                                       [n] * 3, dtype=dtype, impl=reco_space_impl)
        # Geometry, overwriting angle partition
        apart = odl.uniform_partition(0, 8 * 2 * np.pi, n_angles)
        dpart = odl.uniform_partition([-30, -3], [30, 3], [m] * 2)
        geom = odl.tomo.ConeBeamGeometry(apart, dpart, pitch=5,
                                         src_radius=200, det_radius=100)
        # Ray transform
        return odl.tomo.RayTransform(reco_space, geom, impl=ray_trafo_impl)
    else:
...
def test_projector(projector, in_place):
    ...
    assert proj.max() == pytest.approx(expected_max, rel=rtol)
...
def test_angles(projector):
    ...
    if isinstance(projector.geometry, odl.tomo.DivergentBeamGeometry):
        src_to_det = (
            projector.geometry.src_radius #type:ignore
            + projector.geometry.det_radius #type:ignore
        )
        magnification = src_to_det / projector.geometry.src_radius #type:ignore
        expected *= magnification

    assert max_pixel == pytest.approx(expected, abs=0.2)


def test_complex(ray_trafo_impl):
    ...
    ray_trafo_c = odl.tomo.RayTransform(space_c, geom, impl=ray_trafo_impl)
    ray_trafo_r = odl.tomo.RayTransform(space_r, geom, impl=ray_trafo_impl)
...
def test_anisotropic_voxels(geometry):
    ...
    vol_one = ray_trafo.domain.one() #type:ignore
    data_one = ray_trafo.range.one() #type:ignore
